...
async def proxy_drive(request: Request, path: str):
    target_url = f"{PLACEMENT_API_URL}/admin/drive/{path}"
    return await forward_request(request, target_url)

@router.api_route("", methods=["GET", "POST", "PUT", "DELETE", "PATCH", "OPTIONS", "HEAD"])
async def proxy_drive_root(request: Request):
    target_url = f"{PLACEMENT_API_URL}/admin/drive"
    return await forward_request(request, target_url)
